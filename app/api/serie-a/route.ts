...
export async function GET() {
  try {
    const cacheKey = "serie-a-data"

    // Check cache first
    const cachedData = rateLimiter.getCached(cacheKey)
    if (cachedData) {
      console.log("Returning cached Serie A data")
      return NextResponse.json(cachedData)
    }

    // Check rate limit
    if (!rateLimiter.canMakeRequest("serie-a")) {
      const waitTime = rateLimiter.getTimeUntilReset("serie-a")
    ...
    const [matchesResponse, standingsResponse] = await Promise.all([
      fetch(`${BASE_URL}/competitions/SA/matches?status=SCHEDULED,LIVE,IN_PLAY,FINISHED&limit=20`, {
        headers: { "X-Auth-Token": API_KEY },
      }),
      fetch(`${BASE_URL}/competitions/SA/standings`, {
        headers: { "X-Auth-Token": API_KEY },
      }),
    ])

    if (!matchesResponse.ok || !standingsResponse.ok) {
      throw new Error("Failed to fetch Serie A data")
    }
